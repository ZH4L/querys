<template>
  <div class="edit">
    ...
    <div class="ebody">
      <transition-group name="list" tag="p" leave-active-class="animated bounceOut">
      ...
      <transition name="slide-fade" enter-active-class="animated zoomIn"leave-active-class="animated zoomOut"><type v-on:adds="addsin" v-on:addm='addmul' v-on:addt="addtxt" v-if="type"></type></transition>
    ...
    <div class="efoot">
      ...
      <layer :layer="layer" @confirm="public" :content="'确定发布问卷？'" :foot="'发布日期截止为'+ques.deadline"></layer>
      <layer :layer="layer2" :content="'请选择日期'" ></layer>
  </div>
</template>
<script>
  ...
  export default{
    ...
    methods:{
      ...
      confirm(){
            if(this.ques.deadline){
                this.layer.show=true;
            } else{
                this.layer2.show=true
            }
        
      },
      public(){
        this.ques.status="发布中";
        if (!querys.includes(this.ques)) {querys.push(this.ques);}
        router.push('/');
      },
      upDate(arg){
        this.value1=arg;
